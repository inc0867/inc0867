...
def Chat(id):
    ...
    if request.method == "POST" and form.validate():
        ...
        return render_template("Chat.html",id = id,form = form)
...
@app.route("/yorum/<string:id>")
def yorum(id):
    cursor = mysql.connection.cursor()

    sorgu = "Select * From yorumlar where yorumyapılanyer = %s"

    result = cursor.execute(sorgu,(id,))

    if result > 0:
        yorum = cursor.fetchall()
        return render_template("yorum.html",id=id,yorum=yorum)
    else:



        return render_template("yorum.html",id=id) 

@app.route("/yorumy/<string:id>",methods=["GET","POST"])
def yorumyap(id):
    form = yorumyapma(request.form)

    if request.method == "POST" and form.validate():
        yorum = form.yorumy.data

        cursor = mysql.connection.cursor()

        sorgu = "Insert into yorumlar(yorumyapılanyer,yorumuyapan,yorum) Values(%s,%s,%s)"


        cursor.execute(sorgu,(id,session["username"],yorum))
        ...
        return redirect(url_for("paylasım"))
    else:
        return render_template("yorumy.html",form=form)
...
class yorumyapma(Form):
    yorumy = TextAreaField("Yorumyapın",validators=[validators.length(max=400 ,message="Bir yorum en fazla 400 karekter içerebilir")])
